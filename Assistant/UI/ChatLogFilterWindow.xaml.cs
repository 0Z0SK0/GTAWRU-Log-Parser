...
namespace Assistant.UI
{
    ...
    public partial class ChatLogFilterWindow
    {
        ...
        private readonly Dictionary<string, Tuple<string, bool>> _filterCriteria = new Dictionary<string, Tuple<string, bool>>
        {
            ...
            { "IC", Tuple.Create(@"^(\(Car\) ){0,1}[\p{L}]+ {0,1}([\p{L}]+){0,1} (говорит|кричит|шепчет)( \[тихо\]){0,1}:.*$", Properties.Settings.Default.ICCriterionEnabled) },
            ...
            { "PM", Tuple.Create(@"^\(\( PM (к|от) \(\d*\) [\p{L}]+ {0,1}([\p{L}]+){0,1}:.*?\)\)$", Properties.Settings.Default.PMCriterionEnabled) },
